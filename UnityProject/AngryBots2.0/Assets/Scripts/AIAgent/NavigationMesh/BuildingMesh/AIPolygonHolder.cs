...
public class AIPolygonHolder
{
	...
	void checkNeighbors ()
	{
		...
		for (int count = 0; count < polygonsHeld; count ++) {
			if (count != position) {
				if (polygonArray [position].isNeighbor (polygonArray [count]) || polygonArray [position].isNeighbor2 (polygonArray [count])) {
	...
	public void drawPolygons ()
	{
		...
		for (int count = 0; count < polygonArray.Length; count++) {
			if (polygonArray [count] != null) {
				polygonArray [count].drawSelf (Color.yellow);
			}
		}
	}
	...
	void checkAndAddNeighborAtIndex (int neighborToBeIndex)
	{
		for (int count = 0; count < polygonArray.Length; count++) {
			if (polygonArray [count] != null && count != neighborToBeIndex) {
				if (polygonArray [neighborToBeIndex].isNeighbor (polygonArray [count]) || polygonArray [neighborToBeIndex].isNeighbor2 (polygonArray [count])) {// checks to see if This polygon is a neighbor to another
